...
(define core/size (FingerTreeWrap (lambda () 0) (lambda (_) 1) +))
...
(define (ral-delete-ft:impl ft idx depth)
  (void)
)

(define (ral-delete-digit:impl digit idx depth)
  (void)
)

(define (ral-delete-node:impl node idx depth)
  (void)
)

(define (ral-delete-ft ft idx)
  (match-define-values (l v r) (ral-split ft idx))
  (values (ral-append l r) v)
)

(provide ral-delete-ft)
(provide ral? ral-empty?)
(provide ral-take ral-take-right ral-drop ral-drop-right ral-split-at ral-split-at-right)
(provide ral-copy)
...
; (define-match-expander ral (lambda (stx)
