...
(define (measure:node f n depth)
  (match depth
    [0 (match-define (FingerTreeWrap _ m _) f) (m n)]
    [_ (match n
        [(Node2 v _ _) v]
        [(Node3 v _ _ _) v]
      )]
...
(define (measure:ft f ft depth)
  (match ft
    ...
    [(Empty) (match-define (FingerTreeWrap e _ _) f) (e)]
  )
)

(define (measure:digit f d depth)
  (match-define (FingerTreeWrap _ _ as) f)
  (match d
    [(One a)
...
(define (consL:impl core ft v [depth 0])
  (match ft
    [(Empty) (Single v)]
    [(Single a)
      (match-define (FingerTreeWrap _ m as) core)
